...
class KeyRecordApplication(main_overlay.OverlayApplication):
    ...
    def get_monitor(self):
        self.root.update()
        return {
            "left": self.root.winfo_rootx() + self.canvas.winfo_x(),
            "top": self.root.winfo_rooty() + self.canvas.winfo_y(),
    ...
    def screen_prep(self):
        ...
        for k in range(3):
            for j in range(7):        
                self.key_positions.append((curr_x - r, curr_x + r - 1, curr_y - r, curr_y + r - 1, KEYS[k][j]))
                """
